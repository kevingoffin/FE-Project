...
%   sigma_hat  - Estimated volatility (σ)
    ...
    len = size(Rt_IS_NY_Filtered,1);       % Number of observations

    % --- MOMENT CALCULATIONS ---
    Y_min = sum(x_i_minus_1)/len;      % E[R_{t-1}]
    Y_pls = sum(x_i)/len;              % E[R_t]
    
    Y_min_min = sum(x_i_minus_1.^2)/len;  % E[R_{t-1}^2]
    Y_pls_pls = sum(x_i.^2)/len;          % E[R_t^2]
    
    Y_min_plus = (1/len) * sum(x_i_minus_1.*x_i);  % Cross-moment E[R_{t-1}*R_t]
