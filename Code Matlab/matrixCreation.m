function [ann_return_vector, u_star, d_star, mu_lev_stop_loss, trade_time] = matrixCreation(X_OS, c_bar, SIGMA, theta, l_vector, f, scaling_factor, w0, verbose_optimizedReturned)
    ...
    len = length(l_vector);
    ann_return_vector = zeros(len, 1);
    u_star = zeros(len, 1);
    d_star = zeros(len, 1);
    mu_lev_stop_loss = zeros(len, 1);
    trade_time = zeros(len, 1);
    for i = 1:len
        % Calculate optimal trading bands (u*, d*) for current stop-loss
        [u_star(i), d_star(i), ~] = bands(c_bar, l_vector(i), SIGMA, theta, f);

        mu_lev_stop_loss(i) = evaluate_mu(d_star(i), u_star(i), c_bar, theta, l_vector(i), SIGMA, f);
        trade_time(i) = pi*(erfid(d_star(i), l_vector(i))*erfid(u_star(i),d_star(i))/erfid(u_star(i), l_vector(i)));
        
        % Simulate strategy performance with current thresholds
        % Multiplier 6 converts to annualized returns (assuming 6-month period)
        ann_return_vector(i) = optimizedReturn(X_OS, d_star(i), u_star(i), l_vector(i), c_bar, SIGMA, f, scaling_factor, w0, verbose_optimizedReturned);
    end
